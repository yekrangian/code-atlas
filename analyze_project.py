...
def main():
    ...
    # Save raw JSON data
    json_output = "code_graph.json"
    with open(json_output, 'w', encoding='utf-8') as f:
        json.dump(graph_data, f, indent=2)
    print(f"\n✓ Graph data saved to: {json_output}")
    ...
    # HTML visualization
    html_output = visualizer.generate_html_visualization("code_graph.html")
    print(f"✓ Interactive HTML visualization: {html_output}")
    
    # Text report
    report_output = visualizer.generate_text_report("code_analysis_report.txt")
    print(f"✓ Text report: {report_output}")
    
    # DOT format output
    dot_output = visualizer.generate_dot_output("code_graph.dot")
